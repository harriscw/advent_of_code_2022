...
def do_one_sand(acc): #get the final coordinate of a given piece of sand being drop
    ...
    while True:
        ...
        if(x-1 < min(coords.keys()) or x+1 > max(coords.keys())): #you are going over the edge into the abyss
            return(False)
        elif(y+1 not in coords[x]): #if nothing below go there
            y+=1
        elif(y+1 not in coords[x-1]):   #if nothing down & left go there
            x-=1
            y+=1
        elif(y+1 not in coords[x+1]): #if nothing down & right go there
            x+=1
            y+=1
